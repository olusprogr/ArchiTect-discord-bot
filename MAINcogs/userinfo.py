...
class Userinfo(commands.Cog):
    ...
    async def userinfo(self, ctx, user: Option(discord.Member, "Select a user", default=None)):
        ...
        response = requests.get(self.member.avatar.url)
        ...
        embed.set_author(name=f"User Information - {self.member}", icon_url=self.member.avatar.url if self.member.avatar else discord.Embed.Empty)
        embed.set_thumbnail(url=self.member.avatar.url if self.member.avatar else discord.Embed.Empty)

        embed.add_field(name="ID:", value=self.member.id, inline=False)
        embed.add_field(name="Name:", value=self.member.display_name, inline=False)
        embed.add_field(name="Bot:", value=self.member.bot, inline=False)
        embed.add_field(name="Status:", value=str(self.member.status).title(), inline=False)
        embed.add_field(name="Activity:", value=self.member.activity.name if self.member.activity else "None", inline=False)
